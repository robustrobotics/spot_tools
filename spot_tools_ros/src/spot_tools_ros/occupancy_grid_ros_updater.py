...
class OccupancyGridROSUpdater:
    def __init__(self, node: Node, body_frame, odom_frame, occupancy_map: OccupancyMap, feedback, tf_buffer):
        self.node = node
        ...
        self.feedback.print("INFO", f"Initialized occupancy grid updater {self.body_frame} to {self.odom_frame}.")
    ...
    def occupancy_map_callback(self, msg):
        ...
        with self.occupancy_map:
            ...
            odom_to_robot_map = get_tf_pose(self.tf_buffer, self.odom_frame, occupancy_frame_id)
            ...
            self.occupancy_map.set_grid(occ_map, msg.info.resolution, map_origin_odom_frame, robot_pose_homo, self.node.get_clock().now())

            # Publish the inflated occupancy map
            self.publish_inflated_occupancy_map(msg)
        
        self.feedback.print("DEBUG", f"Received occupancy grid: size=({msg.info.width}, {msg.info.height}), resolution={msg.info.resolution}, origin=({msg.info.origin.position.x}, {msg.info.origin.position.y}, {msg.info.origin.position.z})")
