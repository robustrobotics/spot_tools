...
class CompiledShader():
    ...
    def update_images(self, image_1, image_2):
        ...
        if self.image1 is None:
            self.image1 = ImageInsideOpenGL(image_1.image, self.format)
        else:
            self.image1.update(image_1.image)
        if self.image2 is None:
            self.image2 = ImageInsideOpenGL(image_2.image, self.format)
        else:
...
def stitch_live(robot, jpeg_quality_percent=50):
...
def stitch(robot, jpeg_quality_percent=50, crop_image=False):
    ...
    images = image_client.get_image_async(requests, timeout=5.0).result()

    # Prepare images for OpenGL
    front_right = None
    front_left = None
    for image in images:
    ...
    if front_right is None or front_left is None:
        raise ValueError("Failed to retrieve both front fisheye images.")

    # Update shader program with images
    program.update_images(front_right, front_left)
    stitching_camera = StitchingCamera(front_right, front_left)

    # Render the stitched image
    glClearColor(0, 0, 255, 0)
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
    draw_routine(display, program, stitching_camera)

    # Read the rendered image from the OpenGL framebuffer
    width, height = display
    glPixelStorei(GL_PACK_ALIGNMENT, 1)
    pixels = glReadPixels(0, 0, width, height, GL_RGB, GL_UNSIGNED_BYTE)
    image = numpy.frombuffer(pixels, dtype=numpy.uint8).reshape(height, width, 3)
...
def stitch_RGB(fl_img, fr_img, crop_image=False):
    ...
    # Compile shader program
    program = CompiledShader(vert_shader, frag_shader, format="RGB")

    front_right = ImagePreppedForOpenGL(fr_img)
    front_left = ImagePreppedForOpenGL(fl_img)
